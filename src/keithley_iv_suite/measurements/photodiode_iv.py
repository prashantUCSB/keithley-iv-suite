"""Photodiode IV sweep: two-terminal voltage sweep from reverse to forward bias."""
...
from .sweep_config import PhotodiodeConfig
...
def run_photodiode_sweep(
    ...
    """
    Execute a photodiode IV sweep (anode-driven, cathode grounded).

    Parameters
    ----------
    config       : PhotodiodeConfig
    smu          : SMU driving the anode; cathode is grounded externally
    ...
    Returns
    -------
    dict with keys: voltage, voltage_sensed, current, config
    """
    ...
    log.info(
        "Photodiode sweep: V %.3f→%.3f V (%d pts), Ilim=%.3e A",
    ...
    try:
        for idx, voltage in enumerate(v_list):
            if abort_flag[0]:
                log.info("Photodiode sweep aborted at step %d/%d", idx, n_pts)
                break
